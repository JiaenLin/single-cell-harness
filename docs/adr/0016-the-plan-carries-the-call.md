...
2 gaps, 24 claims, 7 documents, 381 tables, 18 units; the paper numbers 90 figures; the panel
page credits 13 plates to the plugin's own plan entries (R11) and the two raw compareInteractions
bars to the tool; the `kinds` criterion is measured on every page and names nothing (R10);
`promised` done here and owing on the reference by the rename's reason, every other verdict and
the unmeasured count the same (R9); R1-R5, R7, R8 as before. The one FAIL is the grader's: R6
looked for "no regression against", the phrase the command prints when something differs
without regressing, and an identical run prints "identical" - the numbers it read were the
...
*Results, 2026-09-13, fifth reproduction (PBS 710985, run
`20260912T163648Z__scprofile-344993d__04_profile__plan`, sealed and graded 17 of 17).* **4f
holds, and with it every step of this record.** R15 and R16 at 0 - the driver log a unit's
maintainer reads charges nothing a unit could not have drawn and reports none of a family's
files as undeclared - and R0-R14 exactly as on 710982: capacity identical to the reference,
432 records, the 6b breakdown, every page count. Minor debt seen on the way, not acted on: the
tool's own seal of a run made from an exported tree reads `commit=unidentified`, because the
export carries `HEAD.txt` and no git; the run key carries the commit and the job's seal
overwrites the tool's, so nothing is lost, but the tool could read `HEAD.txt` where it reads
git.
...
| 4 generated sites, `ctx.rscript`, sites deleted | **done, 4a-4f.** 4e: PBS 710981 sealed 11 of 12, capacity identical to the reference (the one FAIL was the grader's phrase), after 710971 (3 of 10) and 710973 (9 of 11) each found the migration's defects. 4f: PBS 710982 15 of 16 (the feedback's exact-id matching was the finding), PBS 710985 **17 of 17** - see the results under step 4 | harness: 582ff44, b366be4, 51c02e0, 6abdbe9, 4068a81, a6fa330, 091384e; scProfile: the commits after 44a5541 through e41d6f7, then 892593c and 344993d | the companion carries the plan and its interpreter; every one of cellchat's 47 sites is `.draw(id)`; the host launches R; the plugin's R glue is gone; the `plan:<id>` route reaches the plugin's own panels; the manifest half of 4b is 4f (see the amendment) |
