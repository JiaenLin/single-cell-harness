...
| a placed label over a tick or axis label | the label is moved inward, away from that axis's edge, by the overlap plus a pad (added after the first submission: the one class the cohort left as residue) | 40 pt |
...
Written 2026-09-13 after step 7. Three of the reference's numbers were not what they seemed:
the 43 collisions were forty phantom tick labels the old audit measured and matplotlib never
drew, plus three real ones of a single class the repertoire lacked and now has; `measure` was
unanswerable not because the job's shape was wrong but because the floor and the job's counter
were both the wrong instrument, and the instance's own process tree fits in one job (2.2 GB +
13.0 GB per 100k cells, a rate more than twice the declaration because the tree counts the R
worker the floor never saw); and the lookers' 58 defect notes were invisible to the stages
because a look carried no verdict. On the reproduction the audit repaired everything real and
left nothing, the memory terms reached the plugin by one command with no hand on the file, and
the maker's order became look, audit, write. On the replay, cold lookers marked seventy figures
from one printed line, the maker named their findings as the plugin's debt in their words, and
the writer met the round's real finding: the section could still rest on a marked plate while
the claims could not - closed the same day, with four smaller contradictions of the same kind.
Open on purpose: the seventy findings are the plugin's next build, under the top rule, and a
rerun is what clears `audited`; two predictions of mine were wrong about the mechanism's
arithmetic and are recorded as such rather than rewritten.
...
| 5 the reproduction | done: two submissions, 8 of 9 each, the substance held; no third | harness c8c6fc8 (the job), 091f09f (the first record), this commit; scProfile c260046 (the repair the first found missing) | **Second submission, PBS 711029, run `20260913T042903Z__scprofile-c260046__04_profile__audit`: 8 of 9, FAILED.txt again by A9.** A1's restatement held on substance - 3 collisions found on the whole run, 0 remain, every real one repaired (F10's label moved inward on two units; F6's label moved inward, separated from its neighbour and the legend moved out over three passes) - and failed on the arithmetic clause I added to it, "the repairs recorded equal the collisions found": a finding may take several repairs across the passes, so five repairs answered three findings. That clause was a wrong statement about the mechanism, not a defect in it, and it is not worth a third run of the same bytes: the run is complete, the tool's own seal is on it, and it is the replay for steps 6 and 7. Every other prediction held as in the first submission; the fitted terms were the same to the second decimal (2.2 + 13.0). **First submission, PBS 711028, run `20260913T035511Z__scprofile-2b1038e__04_profile__audit`: 8 of 9 held, FAILED.txt by A9's own rule.** A1 failed on its repair count: the audit found 3 collisions on the whole run before repair, not 43 - the reference's forty were tick labels beyond the axis limits, laid out by matplotlib and never drawn (F1's limit is 3,379; its undrawn '4000' was the reported collision), which the old audit measured and the new collector excludes; the lookers' notes on those five kinds mention no overlap. The analysis table above read them as the shrink's; they were the audit's own. The three real collisions were one class - a placed label over a tick label at the axes' edge (F10 twice, F6 once) - which the repertoire lacked; it has it now (`annotation_inward`). A0, A2-A8 held: same 945 names, no repair added a collision, 90 of 90 tables identical, `measure` answered from the process trees (18 points, 3.3 to 10.1 GB, fitted 2.2 GB + 13.0 GB per 100k against a declaration of 3.1 + 5.7 - the tree counts the R worker the floor missed), every tree peak under the job's counter, the maker's order as predicted, identical counts, the environment reused. |
| 6 the fill on cellchat | done | scProfile ad6a691 | `sch dev convert measure --root ~/tools/scProfile --point kernel --name cellchat --run <the replay of 711029> --apply`: one line of `kernels/cellchat.py` changed by the tool, `3.1 + 5.7` to `2.4 + 14.3` (the fit plus 10%, the run key in the trailing comment); the gate then answers on the same replay (exit 0); `scprofile validate` clean. The rate more than doubled because the tree counts the R worker the floor never saw. The plugin's version was not bumped: the terms change no product, and `freshness` was already owing four commits before this one - the author's call, not the tool's. |
| 7 blind 0005 | done: 3 of 5 held, one half, one failed on its count in the right direction; ADR-0018 complete | harness: this commit (the record); scProfile d83c85e and da6033f (the five defects) | two cold lookers marked 70 of 139 figures `--defect` from the printout alone, 0 refusals; the maker named the eye's findings as debt; the writer's claim on a marked plate was refused; five defects found and fixed test-first - the largest that `paper --write` did not wait on the figures while the agenda and `--claim` did; verified on the replay after the fix |
