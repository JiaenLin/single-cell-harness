...
**Date** 2026-09-15. **Follows** ADR-0024 (the figure plan under budget). **Status** complete; the
table at the end records each step. **Stops before** the eight held-out plugins, which
...
| 3 the fresh look | done | two cold lookers on 40 plates (40 carried); the author's second and third answers (scProfile a7f247b = 0.38.0, 789b0b4 = 0.39.0); PBS 711745 (0.38.0, `20260915T023343Z__scprofile-a7f247b__04_profile__rerun`, 110 plates - the one edit failed on every contrast) and PBS 711759 (`20260915T032050Z__scprofile-789b0b4__04_profile__rerun`, 31 min, SEALED, 122 plates); a third looker on the 11 plates the plugin's own ggplot redraws | 40 recorded, 0 refused, 28 marked defect; the worksheet: 33 findings on 12 kinds - the legends rewritten in register had unbound every disclosure a stated answer was tied to, and every finding those had closed came back. The author, in two more exchanges: 22 re-stated in the legends' new words, four legends given one sentence; the two differential heatmaps given a palette name the tool refused ("invalid color name") on every contrast, caught by the run and not by the seal - `capacity --promised` had read the promise as kept because the function drew for another entry (fixed: a promise is per entry); the argument reverted and the key's behaviour disclosed instead. On the final run: the worksheet reads 0 open findings; the third looker marked 4 defects, all closed by the kind's disclosure, and reported that one of them - a label overlap - is not what the disclosure it was closed by says: a kind-level closure hides a different finding. E1 **failed** on the letter: 33 open after the fresh look, not two; 0 after the answers. Recorded, not fixed: the closure by kind |
| 4 the section against the set | done | scProfile 8d9ab65 (the draft record carries the set's digest; the page, `next` and the paper station mark a section NOT CURRENT when the set changed); 9d6cc4d (the hint after `--write` carries the plugin) | The mechanism first, test-first, exercised in the suite; on the live runs the section was re-carried on a new run, so the within-run staleness had no occasion to fire. A cold writer started from the section carried into the earlier run, checked every citation against the new brief and every described plate against the plate, rewrote the interaction section for both directions, added the test's own p-values to every leading-pathway sentence and the differential matrices to every contrast's opening; 1,860 words carried in on the first `--write`, 4 claims, 0 refusals; one defect reported (the printed hint without `--plugin`, fixed). A cold reviewer: 12 of 13 standing, 1 narrowed for a reason the figure shows - the per-arm totals plate is unrestricted while the two-scale ratios are on shared populations. W1 **failed** on the letter (13 standing predicted) |
| 5 the writing seal, the status | done | writing runs `20260915T044935Z__scprofile-9d6cc4d__04_profile__written` (FAILED W3: `looked_at` and `audited` owed on the replay), `20260915T045830Z__scprofile-42230d8__04_profile__written` (3 of 3, `audited` still todo on the replay), `20260915T051032Z__scprofile-c1e92c2__04_profile__written` (PBS 711781, 3 of 3) | The carry is a read-time view across sibling runs and the replay on the cluster has none beside it: 69 looks and 38 stated answers read as never taken. `scprofile review --adopt` (42230d8, d2e4bfb, c1e92c2) appends what a run relies on to its own ledger - by bytes for identical plates, by entry for a kind's stated disclosure whose words still stand - named for the run it came from. On the third seal the maker reads **build 9 of 9, test 8 of 8** on the run's own replay: the first run of the arc with every stage done. M1 held |
| 6 the record | done | this commit; docs/blind/0010-the-last-turn.md; memory | A1 held; R1 held; E1 failed on the letter; W1 failed on the letter; M1 held; B1 **failed**: four mechanism defects fixed test-first (the tests forcing capitals, the promise per entry, the hint without the plugin, the carry that did not travel) and one recorded open (a kind's disclosure closes a different finding). Three reruns for one predicted, three writing seals for one. What is left for the plugin: nothing the loop names. What is left for the mechanism: the closure by kind, and a disclosure re-bound in bulk when a legend is rewritten. The eight held-out plugins are ADR-0026 |
