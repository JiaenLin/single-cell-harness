...
| second (PBS 712313, `20260915T153602Z__scprofile-63c2e1d__04_profile__rerun`) | 3 min 11 s to the gate's refusal (exit 4) | fixture_a FAIL in 147 s: cellchat ran on 9 units and the one contrast (144 s), and `--promised` owed `compareInteractions` - an interaction-axis plate the one-factor design never launches (the reader waives a promise on an axis the run's directories show no occurrence of, scProfile f7e5a0d); fixture_b ok in 1.8 s, **falsely again**, and this time the log said why: the fixture's `plan` had not been told the roles, guessed the sample column, refused for the data, and printed "not ready in this installation" over it because CellChatDB, bundled, was UNRESOLVED for want of a `--references` directory (all three fixed at f7e5a0d: the plan carries the roles; bundled references need no directory; the installation's sentence is the installation's alone) | - | - | - | (1) FAILED as designed - the gate refused the cohort and the refusal's words named what to fix; the seal's `missing=` list also carried the first run's fixture files as expected products (harness 4f33e56: the gate's scratch is not a product); (2)-(4) unmeasured |
| **third, the run for review** (PBS 712314, `20260915T154752Z__scprofile-f7e5a0d__04_profile__rerun`, cellchat 0.42.0, harness 4f33e56) | **14 min 9 s**, the fixture 2 min 28 s of it | **both shapes ok in 147 s, in parallel**: cellchat ran end to end on each synthetic cohort - 6 samples and the 2 arms, 23 unit plates and 10 contrast plates a shape, `--promised` done with `compareInteractions` waived by name; shape b, every column renamed, resolved every role by name | **18 of 18** reuse the saved object under the span's key (forecast HIT) | 122, the same set as the control by name and pixel size; the re-stated legend on the paper page | build 9 of 9; measure, cores, cost, declared (6 tables a unit, no drift), promised done; looked_at, audited, written, delivered todo | (1) held; (2) held; (3) held, no fixture file among the products; (4) held - the four owing stages are the ones the prediction named, for the reason it gave |

Three jobs for one, and the two the gate refused or misread were the point: a gate that had
never existed found, on its first two runs, four things about the tool and one about the job's
