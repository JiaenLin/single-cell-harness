#!/bin/bash
# The audit reproduction (harness ADR-0018 step 5), cellchat only. Never qsub the job directly:
...
REF="${REF:-$PROJ/runs/scprofile/04_profile/20260912T163648Z__scprofile-344993d__04_profile__plan}"
...
RUNKEY="${STAMP}__scprofile-${COMMIT}__04_profile__audit"
...
"$HOME_/tools/hpc-site/validate.sh" "$HARNESS/jobs/audit_reproduction.pbs"
JOB=$(qsub -o "$RUNDIR/logs/" -e "$RUNDIR/logs/" \
     -v RUNDIR="$RUNDIR",RUNKEY="$RUNKEY",REF="$REF",HARNESS="$HARNESS" \
     "$HARNESS/jobs/audit_reproduction.pbs")
