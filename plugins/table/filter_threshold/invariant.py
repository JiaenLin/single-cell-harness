#!/usr/bin/env python3
"""Companion: every masked row is named in the removal record, and only those."""
...
def register(inv):
    ...
    @inv.check("the mask covers every observation")
    def _(run):
        for c in run.contributions():
            if c["slot"] == "mask" and c["coverage"] != run.observation_count():
                inv.fail(f"mask {c['name']} covers {c['coverage']} of {run.observation_count()}")
